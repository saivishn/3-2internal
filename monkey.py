def solve(banana,box,height,monkey,hold):
    ...
    options={1:"Move to box", 2:"Hold box to move",3:"Move to banana", 4:"Climb onto the box", }
    for option in options:
        if option==1 and hold==0 and height==0 and solve(banana,box,height,box,hold):
            ans.append(options[option])
            found=1
            break
        elif option==2 and height==0 and monkey==box and solve(banana,box,height,monkey,1):
            ans.append(options[option])
            found=1
            break
        elif option==3 and height==0 and ((hold==1 and solve(banana,banana,height,banana,hold)) or (hold==0 and solve(banana,box,height,banana,hold))):
            ans.append(options[option])
            found=1
            break
        elif option==4 and height==0 and monkey==box and solve(banana,box,height+1,monkey,0):
            ans.append(options[option])
            found=1
            break
